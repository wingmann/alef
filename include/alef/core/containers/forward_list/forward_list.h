...
public:
    ...
    using iterator        = iterators::forward_iterator<value_type, node_type>;
    using const_iterator  = iterators::forward_iterator<const value_type, node_type>;
...
public:
        ...
        : size_{static_cast<size_type>(std::distance(first, last))}
    {
        ...
        for (size_type index = 0; index < size_ - 1; ++index) {
            current_node->next = new forward_list_node<T>(*(++first));
            current_node = current_node->next;
        }
        tail_ = current_node;
        tail_->next = nullptr;
